...
from Utilities.utils import get_course_level
...
class Course(AbstractCanvasObject):
    def __init__(self, obj, in_as='json'):
        ...
        if in_as == 'json':
            ...
            self._course_level = get_course_level(obj)
            ...
            self._end_date = None
            self._json = None
            self._enrollment_term_id, self._id, self._course_level, self._name, start_at_string, end_at_string = obj.split(',')
            self._set_start_end_dates(start_at_string, end_at_string)
        else:
            raise ValueError("in_as must by in ['json', csv'] or empty which defaults to json")

    def as_csv_line(self):
        return f'{self._enrollment_term_id},{self._id},{self._name},{self._course_level},{self._start_date},{self._end_date}\n'
